(function(exports) {
  ...
  var Petr = exports.Petrovich = function(lastName, firstName, middleName, gender) {
    if (lastName.constructor == String && arguments.length == 1) {
      this._gender = lastName;
    } else
    if (lastName.constructor != String) {
      this._lastName     = lastName.lastName;
      this._firstName    = lastName.firstName;
      this._middleName   = lastName.middleName;
      this._gender       = firstName || Petr.detectGender(lastName);
    } else {
      this._lastName     = lastName;
      this._firstName    = firstName;
      this._middleName   = middleName;
      this._gender       = firstName || Petr.detectGender(lastName, firstName, middleName);
    }
  }
  ...
  var mods = { };
  for (var i = 1; i < Petr.CASES.length; i++) {
    mods[Petr.CASES[i]] = i - 1;
  }
  /**
  ...
  for (var i = 0; i < funcNames.length; i++) {
    (function(funcName) {
      ...
      Petr[funcName] = function(name, gcase, gender) {
        if (this && (this['_' + funcName] !== undefined) && (arguments.length < 3)) {
          gcase = name;
          gender = gender || this._gender;
          name = this['_' + funcName];
        } else {
